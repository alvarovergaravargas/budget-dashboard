...
export const ExecutionGauge = ({ rate = 0, remaining = 0, total = 0 }) => {
  ...
  // Recharts RadialBar needs value > 0 to render
      ...
      <div style={{ position:'relative', height:200 }}>
        <ResponsiveContainer width="100%" height="100%">
          <RadialBarChart cx="50%" cy="80%" innerRadius="60%" outerRadius="90%" startAngle={180} endAngle={0} data={data}>
            ...
            <RadialBar background={{ fill:'var(--bg-panel)' }} dataKey="value" cornerRadius={8} fill={color} />
          </RadialBarChart>
        </ResponsiveContainer>
        <div style={{ position:'absolute', bottom:30, left:0, right:0, textAlign:'center' }}>
          <div style={{ fontFamily:'var(--font-display)', fontSize:38, fontWeight:800, color, lineHeight:1, letterSpacing:'-0.03em' }}>{fmtPct(safeRate)}</div>
