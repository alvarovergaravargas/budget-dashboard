...
export const NecesidadChart = ({ data }) => {
  ...
  const sorted = [...data].sort((a,b) => NECESIDAD_ORDER.indexOf(a.name) - NECESIDAD_ORDER.indexOf(b.name));
          ...
          <div style={{ flex:1, display:'flex', flexDirection:'column', gap:10 }}>
            {sorted.map((d,i) => (
              <div key={d.name}
                ...
                <div style={{ width:28, height:28, borderRadius:8, background:`color-mix(in srgb,${d.color} 18%, transparent)`, border:`1px solid ${d.color}50`, display:'flex', alignItems:'center', justifyContent:'center', fontSize:12, color:d.color, flexShrink:0 }}>{ICONS[d.name]||'?'}</div>
                <div style={{ flex:1 }}>
