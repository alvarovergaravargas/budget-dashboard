        ...
        <div style={{ display:'flex', gap:6 }}>
          {necOptions.map(n => (
            <button key={n||'all'} onClick={() => setFilterNec(n)} style={{
              ...
              background: filterNec===n ? (n ? `color-mix(in srgb,${necesidadColor(n)} 15%, transparent)` : 'var(--accent-dim)') : 'transparent',
              borderColor: filterNec===n ? (n ? necesidadColor(n)+'66' : 'var(--border-accent)') : 'var(--border)',
              color: filterNec===n ? (n ? necesidadColor(n) : 'var(--accent)') : 'var(--text-muted)',
            }}>{n || 'Todos'}</button>
          ))}
        </div>
      </div>
          ...
          return (
            ...
            >
              ...
              <div style={{ display:'flex', alignItems:'center' }}>
                <span style={{ background:`color-mix(in srgb,${catColor} 15%,transparent)`, color:catColor, borderRadius:99, padding:'2px 10px', fontSize:11, whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis', maxWidth:'100%' }}>{tx.categoria}</span>
              </div>
              ...
              <div style={{ textAlign:'center', display:'flex', alignItems:'center', justifyContent:'center', gap:5 }}>
                <span style={{ background:`color-mix(in srgb,${necColor} 15%,transparent)`, color:necColor, borderRadius:99, padding:'2px 10px', fontSize:11, fontWeight:600, display:'flex', alignItems:'center', gap:4 }}>
