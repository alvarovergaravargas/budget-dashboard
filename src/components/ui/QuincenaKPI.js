    ...
    <div style={{
      ...
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:16, gap:8 }}>
        <div style={{ minWidth:0 }}>
          ...
          <div style={{ fontFamily:'var(--font-display)', fontSize: isMobile ? 17 : 22, fontWeight:800, color, letterSpacing:'-0.02em', whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis' }}>{currentQ} · {meta?.short}</div>
        </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:8 }}>
        {[
          ...
          <div key={item.label} style={{ background:'var(--bg-panel)', borderRadius:10, padding: isMobile ? '8px 10px' : '10px 14px' }}>
            <div style={{ fontSize:10, color:'var(--text-muted)', fontFamily:'var(--font-body)', marginBottom:4 }}>{item.label}</div>
            <div style={{ fontFamily:'var(--font-mono)', fontSize: isMobile ? 11 : 13, color:item.color, fontWeight:600 }}>{item.value}</div>
          </div>
        ))}
      </div>
    </div>
